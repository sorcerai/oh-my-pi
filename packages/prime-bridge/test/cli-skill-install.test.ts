...
async function packedPackageOutput(): Promise<string> {
	const packageRoot = path.resolve(import.meta.dir, "..");
	const packProcess = Bun.spawn(["bun", "pm", "pack", "--dry-run"], {
		cwd: packageRoot,
		stderr: "pipe",
		stdout: "pipe",
	});
	const output = await new Response(packProcess.stdout).text();
	if ((await packProcess.exited) !== 0) throw new Error(`bun pm pack failed:\n${output}`);
	return output;
}
