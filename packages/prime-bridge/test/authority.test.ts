 ...
 * tested by nothing.
 *
 * Ported from feat/prime-bridge-authority onto the v18 bridge. The upstream
 * branch also gated a /v1/tool-host WebSocket upgrade behind the supervisor
 * role; this port has no tool-host surface, so that proof has no route to
 * exercise here. Every other gate is unchanged.
 */
...
describe("bridge authority proofs", () => {
	it("proof 1: discovery is session-allowlisted", async () => {
		...
		// The worker's own session passes the gate; 404 only because no host has
		...
		expect(
			(
				await v1(server, "/v1/peers", WORKER_TOKEN, {
					...
					body: JSON.stringify({ targetHarness: "omp", id: "s" }),
				})
			).status,
		).toBe(403);
