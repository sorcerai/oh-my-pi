...
class BridgeSkillTests(unittest.TestCase):
    ...
    def test_wait_acknowledges_claim_and_returns_inner_message(self):
        claim = {
            "message": {"meshMessageId": "message-id", "body": "hello"},
            "claimToken": "claim-token",
            "claimedUntilMs": 1234567890,
        }
        responses = iter([claim, {"ok": True}])

        def urlopen(request, timeout=None):
            self.calls.append((request, timeout))
            return _Response(next(responses))

        client = self._client(urlopen=urlopen)
        result = asyncio.run(client.wait(timeout=1000))

        self.assertEqual(result, claim["message"])
        self.assertEqual(
            [request.full_url for request, _ in self.calls],
            ["http://127.0.0.1:3210/v1/wait", "http://127.0.0.1:3210/v1/wait/ack"],
        )
        self.assertEqual(
            json.loads(self.calls[1][0].data.decode("utf-8")),
            {"claimToken": "claim-token"},
        )
        self.assertEqual(self.calls[0][0].get_header("Authorization"), "Bearer secret-token")
        self.assertEqual(self.calls[1][0].get_header("Authorization"), "Bearer secret-token")

    def test_wait_ack_failure_releases_claim_and_surfaces_error(self):
        claim = {
            "message": {"meshMessageId": "message-id", "body": "hello"},
            "claimToken": "claim-token",
            "claimedUntilMs": 1234567890,
        }
        responses = iter([claim, {"ok": False}, {"ok": True}])
        ...
        client = self._client(urlopen=urlopen)
        with self.assertRaises(omp_message.BridgeProtocolError):
            asyncio.run(client.wait())

        self.assertEqual(
            [request.full_url for request, _ in self.calls],
            [
                "http://127.0.0.1:3210/v1/wait",
                "http://127.0.0.1:3210/v1/wait/ack",
                "http://127.0.0.1:3210/v1/wait/release",
            ],
        )
        self.assertEqual(
            json.loads(self.calls[2][0].data.decode("utf-8")),
    ...
    def test_wait_rejects_malformed_claim_and_releases_valid_claim(self):
        claim = {
        ...
        self.assertEqual(
            [request.full_url for request, _ in self.calls],
            ["http://127.0.0.1:3210/v1/wait", "http://127.0.0.1:3210/v1/wait/release"],
        )

    def test_wait_rejects_malformed_release_response_without_masking_claim_error(self):
        claim = {
            "message": "not-an-object",
            "claimToken": "claim-token",
            "claimedUntilMs": 1234567890,
        }
        responses = iter([claim, {"ok": "yes"}])
        ...
        client = self._client(urlopen=urlopen)
        with self.assertRaises(omp_message.BridgeProtocolError) as raised:
            asyncio.run(client.wait())

        self.assertIn("wait claim", str(raised.exception))
