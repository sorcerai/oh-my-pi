...
class BridgeClient:
    ...
    async def wait(self, from_peer: str | None = None, timeout: int | None = None) -> JsonObject | None:
        ...
        try:
            if not isinstance(value, dict):
                raise BridgeProtocolError("bridge wait response must be a claim object or null")
            candidate_token = value.get("claimToken")
            if isinstance(candidate_token, str) and candidate_token.strip():
                claim_token = candidate_token
            else:
                raise BridgeProtocolError("bridge wait claim must have a non-empty claimToken")
            message = value.get("message")
            if not isinstance(message, dict):
                raise BridgeProtocolError("bridge wait claim message must be an object")
            if "claimedUntilMs" in value:
                claimed_until = value["claimedUntilMs"]
                if (
                    isinstance(claimed_until, bool)
                    or not isinstance(claimed_until, (int, float))
                    or not math.isfinite(claimed_until)
                    or claimed_until < 0
                ):
                    raise BridgeProtocolError("bridge wait claim lease must be finite and non-negative")
            ...
            if not isinstance(acknowledged, dict) or acknowledged.get("ok") is not True:
                raise BridgeProtocolError("bridge wait claim acknowledgement failed")
            return message
        except Exception:
