...
`timeout` is in milliseconds. `wait()` returns the claimed inner message or
`None` on timeout. A received claim is acknowledged before the inner message
is returned. If claim validation or acknowledgement fails, the client makes a
best-effort release of the valid claim and then raises the original error.
`inbox(peek=False)` consumes messages. Use `peek=True` to inspect without
consuming. Responses and receipts are returned without dropping unknown
fields, including `None` values.
...
```text
omp_message peers
omp_message send OMP_SESSION "Please check the result" --session-id PRIME_SESSION
