...
if (WORKER_MODE) {
	...
	test("AuthStorage.createExisting enforces the expected database identity", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "pi-ai-create-existing-identity-"));
		const dbPath = path.join(tempDir, "agent.db");
		try {
			const initialized = await openStorage(dbPath);
			initialized.storage.close();
			...
			await expect(
				AuthStorage.createExisting(dbPath, {}, { dev: expected.dev, ino: expected.ino + 1 }),
			).rejects.toBeInstanceOf(ConfigurationError);
		} finally {
			await removeWithRetries(tempDir);
		}
	...
	test("create-only batch insertion skips existing providers and rolls back malformed input", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "pi-ai-create-only-batch-"));
		const dbPath = path.join(tempDir, "agent.db");
		const live = await openStorage(dbPath);
		try {
			...
			try {
				const result = importer.insertCredentialsIfProvidersAbsent([
					...
					{
						provider: "batch-absent-provider",
						credentials: [{ type: "api_key", key: "batch-absent-secret" }],
					},
				]);
				expect(result).toEqual({
					inserted: ["batch-absent-provider"],
					skipped: ["batch-existing-provider"],
				});
				expect(JSON.stringify(result)).not.toContain("batch-existing-secret");
				expect(JSON.stringify(result)).not.toContain("batch-absent-secret");
				expect(live.store.listAuthCredentials("batch-existing-provider")).toEqual([
					expect.objectContaining({ credential: { type: "api_key", key: "batch-existing-secret" } }),
				]);
				expect(live.store.listAuthCredentials("batch-absent-provider")).toEqual([
					expect.objectContaining({ credential: { type: "api_key", key: "batch-absent-secret" } }),
				]);

				const beforeMalformed = live.store.listAuthCredentials();
				const malformed = {
					type: "oauth",
					access: "valid-access",
					refresh: 42,
				...
				expect(() =>
					importer.insertCredentialsIfProvidersAbsent([
						...
						{ provider: "batch-malformed-provider", credentials: [malformed] },
					]),
				).toThrow();
				expect(live.store.listAuthCredentials()).toEqual(beforeMalformed);
			} finally {
				importer.close();
			}
		} finally {
	...
	test("create-only batch hydrates all-skipped providers with one generation update", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "pi-ai-create-only-batch-cache-"));
		const dbPath = path.join(tempDir, "agent.db");
		const live = await openStorage(dbPath);
		try {
	...
	test("create-only batch rejects duplicate providers before any mutation", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "pi-ai-create-only-batch-duplicates-"));
		const dbPath = path.join(tempDir, "agent.db");
		const { storage } = await openStorage(dbPath);
		try {
	...
	test("create-only insertion rechecks unsafe companions at the write boundary", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "pi-ai-create-only-boundary-"));
		const dbPath = path.join(tempDir, "agent.db");
		const live = await openStorage(dbPath);
		try {
			...
			try {
				...
				importer.close();
				await fs.rm(companionPath, { force: true });
			}
		} finally {
			live.storage.close();
			await removeWithRetries(tempDir);
