...
export class CredentialPool implements CredentialsApi {
	 ...
	 * @param notifyGeneration - When false, the caller batches the generation bump
	 * @returns Whether the cached rows changed
	 */
	replace(provider: string, credentials: StoredCredential[], notifyGeneration = true): boolean {
		...
		if (notifyGeneration) this.bump("credentials");
		return true;
	}
	...
	insertIfProviderAbsent(provider: string, credentials: AuthCredential[]): CredentialInsertResult {
		...
		const outcome = insert.call(this.#store, provider, this.dedupe(provider, credentials));
		this.replace(
			provider,
			outcome.rows.map(record => ({ id: record.id, credential: record.credential })),
		);
		if (outcome.inserted) this.reset(provider);
		return {
	...
	insertIfProvidersAbsent(batch: CredentialBatchInsert[]): CredentialBatchInsertResult {
		...
		const deduped = batch.map(entry => ({
			...
			credentials: this.dedupe(entry.provider, entry.credentials),
		}));
		...
		for (const provider of [...outcome.inserted, ...outcome.skipped]) {
			...
			if (insertedProviders.has(provider)) this.reset(provider);
		}
		if (cacheChanged) this.bump("credentials");
		return outcome;
	}
