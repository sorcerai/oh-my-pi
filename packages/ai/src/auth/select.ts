...
export type TryOAuthOptions = {
	...
	/** Durable row id for target-only resolution; `options.forceRefresh` then re-mints only this row. */
	credentialId?: number;
};
...
export class CredentialSelector {
	...
	async tryOAuth(
		...
		const credentialId = targetCredentialId ?? this.#deps.pool.entries(provider)[selection.index]?.id;
		const forceRefresh = targetCredentialId !== undefined && options?.forceRefresh === true;
		const refreshInput = forceRefresh ? { ...selection.credential, expires: 0 } : selection.credential;
