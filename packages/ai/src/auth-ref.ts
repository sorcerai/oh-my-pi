...
export function parseLocalAuthRef(authRef: string, expectedProviderId: string): LocalAuthRef {
	...
	if (parts[0] === "provider") {
		...
		if (parts.length !== 2 || providerId === undefined || providerId.length === 0) {
			throw new Error(INVALID_AUTH_REF_MESSAGE);
		}
		parsed = { kind: "provider", providerId };
	} else if (parts[0] === "oauth-credential") {
		const providerId = parts[1];
		const credentialIdText = parts[2];
		if (
			parts.length !== 3 ||
			providerId === undefined ||
			providerId.length === 0 ||
			credentialIdText === undefined ||
			!/^[1-9]\d*$/.test(credentialIdText)
		) {
			throw new Error(INVALID_AUTH_REF_MESSAGE);
		}
