...
export class AuthStorage {
	...
	/** Skip construction-time store hygiene writes (see {@link AuthStorage.createExisting}). */
	readonly #skipStoreHygiene: boolean;

	/**
	 * @param skipStoreHygiene - Skip the construction-time expired cache/block
	 *   cleanup writes (used by {@link AuthStorage.createExisting}).
	 */
	constructor(store: AuthCredentialStore, options: AuthStorageOptions = {}, skipStoreHygiene = false) {
		this.#options = options;
		this.#skipStoreHygiene = skipStoreHygiene;
	...
	#compose(store: AuthCredentialStore, sourceLabel: string | undefined): AuthStorageModules {
		...
		const skipStoreHygiene = this.#skipStoreHygiene;
		...
		const usageCache = new UsageCache(store, pool, usageProviders, skipStoreHygiene);
		const blocks = new CredentialBlocks({
			store,
			pool,
			health: blockHealth,
			usageCache,
	...
	/**
	 * Flat redirect kept for fork callers (Prime import) and their prototype spies.
	 * @deprecated Use {@link AuthStorage.credentials}`.insertIfProviderAbsent`.
	 */
	insertCredentialsIfProviderAbsent(provider: string, credentials: AuthCredential[]): CredentialInsertResult {
		return this.credentials.insertIfProviderAbsent(provider, credentials);
	}

	/**
	 * Flat redirect kept for fork callers (Prime import) and their prototype spies.
	 * @deprecated Use {@link AuthStorage.credentials}`.insertIfProvidersAbsent`.
	 */
	insertCredentialsIfProvidersAbsent(batch: CredentialBatchInsert[]): CredentialBatchInsertResult {
		return this.credentials.insertIfProvidersAbsent(batch);
	}
}
