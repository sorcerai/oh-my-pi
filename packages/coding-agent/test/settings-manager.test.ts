...
import { cfgExtensions, cfgSkills } from "@oh-my-pi/pi-coding-agent/extensibility/settings";
import {
	cfgPrimeBridgeAutoStart,
	cfgPrimeBridgeEnabled,
	cfgPrimeBridgeTokenPath,
	cfgPrimeBridgeToolHostAllowTools,
	cfgPrimeBridgeToolHostApprovalTimeoutMs,
	cfgPrimeBridgeToolHostDefaultTools,
	cfgPrimeBridgeToolHostEnabled,
	cfgPrimeBridgeToolHostSessionId,
	cfgPrimeBridgeUrl,
	validatePrimeBridgeApprovalTimeoutMs,
} from "@oh-my-pi/pi-coding-agent/integrations/prime-bridge/settings";
import { cfgMemoriesPhase1InputTokenLimit } from "@oh-my-pi/pi-coding-agent/memories/settings";
import {
...
describe("Settings", () => {
	...
	it("skips create-only mutations beneath scalar ancestors", async () => {
		await writeSettings({ compaction: false, modelRoles: "configured" });
		const settings = await Settings.init({ cwd: projectDir, agentDir });
		const before = await Bun.file(getConfigPath()).text();

		const result = await settings.applyCreateOnly([
		...
		expect(result).toEqual({
			applied: [],
			skipped: ["compaction.enabled", "modelRoles:new"],
		});
		expect(await Bun.file(getConfigPath()).text()).toBe(before);
		expect(await readSettings()).toEqual({ compaction: false, modelRoles: "configured" });
	});

	it("inserts create-only values over a soft-pinned default and releases the pin", async () => {
		const settings = Settings.isolated();
		cfgCompactionEnabled.pinDefault(settings);

		const result = await settings.applyCreateOnly([{ path: "compaction.enabled", value: false }]);

		expect(result).toEqual({ applied: ["compaction.enabled"], skipped: [] });
	...
	describe("Phase 5 settings schema additions", () => {
		it("resolves Prime Bridge defaults for the client settings", () => {
			...
			expect(cfgPrimeBridgeEnabled.get(settings)).toBe(false);
			expect(cfgPrimeBridgeUrl.get(settings)).toBeUndefined();
			expect(cfgPrimeBridgeTokenPath.get(settings)).toBeUndefined();
			expect(cfgPrimeBridgeAutoStart.get(settings)).toBe(false);
		});
		...
		it("resolves configured Prime Bridge client settings", () => {
			...
			expect(cfgPrimeBridgeEnabled.get(settings)).toBe(true);
			expect(cfgPrimeBridgeUrl.get(settings)).toBe("http://127.0.0.1:8787");
			expect(cfgPrimeBridgeTokenPath.get(settings)).toBe("/run/user/501/prime.token");
			expect(cfgPrimeBridgeAutoStart.get(settings)).toBe(true);
		});

		it("resolves Prime Bridge tool-host defaults", () => {
			const settings = Settings.isolated();

			expect(cfgPrimeBridgeToolHostEnabled.get(settings)).toBe(false);
			expect(cfgPrimeBridgeToolHostAllowTools.get(settings)).toEqual([]);
			expect(cfgPrimeBridgeToolHostDefaultTools.get(settings)).toBe(true);
			expect(cfgPrimeBridgeToolHostSessionId.get(settings)).toBeUndefined();
			expect(cfgPrimeBridgeToolHostApprovalTimeoutMs.get(settings)).toBe(60_000);
		});
		...
		it("resolves configured Prime Bridge tool-host settings", () => {
			...
			expect(cfgPrimeBridgeToolHostEnabled.get(settings)).toBe(true);
			expect(cfgPrimeBridgeToolHostAllowTools.get(settings)).toEqual(["read", "grep"]);
			expect(cfgPrimeBridgeToolHostDefaultTools.get(settings)).toBe(false);
			expect(cfgPrimeBridgeToolHostSessionId.get(settings)).toBe("session-fixed");
			expect(cfgPrimeBridgeToolHostApprovalTimeoutMs.get(settings)).toBe(1);
		});
		...
		it("rejects Prime Bridge approval timeouts outside the supported bounds", () => {
			...
			expect(() => Settings.isolated({ "primeBridge.toolHost.approvalTimeoutMs": 60_001 })).toThrow(
				"Prime bridge approvalTimeoutMs must be between 1 and 60000 milliseconds",
			);
		});

		it("defaults skills prompt exposure through the combined skills handle", () => {
			const skills = cfgSkills.get(Settings.isolated());

			expect(skills.promptMode).toBe("all");
			expect(skills.promptSkills).toEqual([]);
		});

		it("resolves configured skills prompt exposure through the combined skills handle", () => {
			const skills = cfgSkills.get(
				Settings.isolated({
		...
		it("exposes the phase-one input token limit through its typed handle", () => {
			expect(cfgMemoriesPhase1InputTokenLimit.get(Settings.isolated())).toBe(4000);
		});
	});
