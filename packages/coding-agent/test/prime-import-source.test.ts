...
describe("discoverPrimeSource", () => {
	...
	it("keeps settings-only revalidation within budgets when an out-of-domain model is added", async () => {
		const root = await temporaryDirectory();
		const sourceRoot = path.join(root, "prime");
		const cwd = path.join(root, "project");
		const settings = '{"stable":true}\n';
		await writeText(path.join(sourceRoot, "settings.json"), settings);
		const first = await discoverPrimeSource({
			sourceRoot,
			cwd,
			sessionRoot: root,
			maxTotalBytes: settings.length,
			maxEntries: 2,
		});
		expect(first.losses).toEqual([]);
		expect(first.snapshot.files.map(file => file.sourceRef)).toEqual(["global/settings.json"]);

		await writeText(path.join(sourceRoot, "models.json"), `${"model".repeat(settings.length)}\n`);
		expect(await revalidatePrimeSource(first.snapshot, { domains: ["settings"] })).toEqual({ ok: true, losses: [] });
	});
