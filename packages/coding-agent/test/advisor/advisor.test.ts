...
describe("advisor", () => {
	...
	describe("AdvisorRuntime quota classification", () => {
		...
		it("pauses when host recovery keeps cycling between quota-exhausted models", async () => {
			const promptInputs: Array<string | AgentMessage[]> = [];
			...
			const agent: AdvisorAgent = {
				prompt: async input => {
					promptInputs.push(input);
					// Yield so a broken recovery loop cannot starve the regression's stop condition.
					const yielded = Promise.withResolvers<void>();
					setImmediate(yielded.resolve);
					await yielded.promise;
					throw new Error("429 Monthly usage limit reached. Resets in 1 day.");
				},
				abort: () => {},
				reset: () => {},
				state: { messages: [] },
			...
			const host: AdvisorRuntimeHost = {
				...
				onTurnError: () => {
					model = model === "provider/first" ? "provider/second" : "provider/first";
					return true;
				},
				notifyQuotaExhausted: () => {
					quotaNotifications++;
			...
			const runtime = new AdvisorRuntime(agent, host, 0);
			try {
				const messages: AgentMessage[] = [{ role: "user", content: "quota-turn", timestamp: 1 }];
				runtime.onTurnEnd(messages);
				await settleUntil(() => runtime.quotaExhausted || promptInputs.length >= 12);

				expect(runtime.quotaExhausted).toBe(true);
				expect(quotaNotifications).toBe(1);
				expect(runtime.yielded).toBe(true);
				expect(runtime.backlog).toBeGreaterThan(0);
				const attemptsAtPause = promptInputs.length;
				messages.push({ role: "user", content: "later turn", timestamp: 2 });
				runtime.onTurnEnd(messages);
				await runtime.waitForCatchup(1000, 1);
				expect(promptInputs).toHaveLength(attemptsAtPause);
			} finally {
				runtime.dispose();
			}
		});
		it("bounds quota recovery when prompt resolves with a provider error", async () => {
			const promptInputs: Array<string | AgentMessage[]> = [];
			let recoveryCalls = 0;
			let quotaNotifications = 0;
			const state: AdvisorAgent["state"] = { messages: [] };
			const agent: AdvisorAgent = {
				prompt: async input => {
					promptInputs.push(input);
					const yielded = Promise.withResolvers<void>();
					setImmediate(yielded.resolve);
					await yielded.promise;
					state.error = "429 Monthly usage limit reached. Resets in 1 day.";
				},
				abort: () => {},
				reset: () => {},
				state,
			...
			const runtime = new AdvisorRuntime(
				...
				{
					snapshotMessages: () => [],
					getModelIdentity: () => "provider/same",
					onTurnError: () => {
						recoveryCalls++;
						return true;
					},
		...
		it("clears quota recovery budget after a successful advisor turn", async () => {
			const promptInputs: Array<string | AgentMessage[]> = [];
			let recoveryCalls = 0;
			let promptCalls = 0;
			const messages: AgentMessage[] = [{ role: "user", content: "first", timestamp: 1 } as AgentMessage];
			const agent: AdvisorAgent = {
				prompt: async input => {
					promptInputs.push(input);
					promptCalls++;
					if (promptCalls === 3 || promptCalls === 6) return;
					throw new Error("insufficient_quota: rate limit exceeded");
				},
				abort: () => {},
				reset: () => {},
				state: { messages: [] },
			...
			try {
				...
				messages.push({ role: "user", content: "second", timestamp: 2 } as AgentMessage);
				runtime.onTurnEnd(messages);
				await settleUntil(() => promptCalls === 6 && runtime.backlog === 0);
		...
		it("restores the quota recovery budget on explicit reset", async () => {
			const promptInputs: Array<string | AgentMessage[]> = [];
			let recoveryCalls = 0;
			let quotaNotifications = 0;
			const messages: AgentMessage[] = [{ role: "user", content: "quota-turn", timestamp: 1 } as AgentMessage];
			...
			try {
				runtime.onTurnEnd(messages);
				await settleUntil(() => runtime.quotaExhausted && promptInputs.length === 3);
				expect(recoveryCalls).toBe(2);

				runtime.reset();
				runtime.onTurnEnd(messages);
				await settleUntil(() => runtime.quotaExhausted && promptInputs.length === 6);

				expect(recoveryCalls).toBe(4);
				expect(quotaNotifications).toBe(2);
			} finally {
				runtime.dispose();
			}
		});
