...
describe("AdvisorEmissionGuard", () => {
	...
	it("dedupes replayed advice across seats without sharing update budgets", () => {
		const history = new AdvisorEmissionHistory();
		const first = new AdvisorEmissionGuard(history, 1);
		const second = new AdvisorEmissionGuard(history, 1);

		first.beginUpdate();
		second.beginUpdate();
		expect(first.admit("Missing rollback", { rank: 1, pending: false })).toEqual({ accepted: true });
		expect(second.admit("missing rollback!", { rank: 1, pending: false })).toEqual({
			accepted: false,
			reason: "duplicate",
		});
		expect(second.admit("Add a falsifier check", { rank: 1, pending: false })).toEqual({ accepted: true });
		first.beginUpdate();
		expect(first.admit("Review the shutdown path", { rank: 1, pending: false })).toEqual({ accepted: true });
	...
	it("honors blockers, configured budgets, and the global budget cap", () => {
		const guard = new AdvisorEmissionGuard({ budgetPerUpdate: 1 });
		expect(guard.admit("Minor naming nit.", { rank: 1, pending: false })).toEqual({ accepted: true });
		expect(guard.admit("Destructive migration will drop user data.", { rank: 3, pending: false })).toEqual({
			accepted: true,
		});
		expect(guard.admit("Destructive migration will drop user data.", { rank: 3, pending: false })).toEqual({
			accepted: false,
		...
		expect(guard.admit("Stop.", { rank: 3, pending: false })).toEqual({ accepted: false, reason: "noise" });

		const capped = new AdvisorEmissionGuard({ budgetPerUpdate: 100 });
		for (let i = 0; i < ADVISOR_MAX_BUDGET_PER_UPDATE; i++) {
			expect(capped.admit(`Note ${i}`, { rank: 2, pending: false }).accepted).toBe(true);
		}
		expect(capped.admit("Note 33", { rank: 2, pending: false })).toEqual({
			accepted: false,
			reason: "rate-limit",
		});
	});
