...
class GatedBackend implements SessionStorageBackend {
	...
	async writeFullCreateOnly(path: string, content: string, _mtimeMs: number, _title?: unknown): Promise<void> {
		if (this.failWrites > 0) {
			this.failWrites -= 1;
			throw new Error("transient backend write failure");
		}
		if (this.files.has(path)) {
			throw Object.assign(new Error(`File exists: ${path}`), { code: "EEXIST" });
		}
		this.files.set(path, content);
	}
