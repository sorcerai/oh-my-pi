...
describe("AgentSession.refreshExtensionTools (Task 0 Slice B)", () => {
	...
	it("keeps detached callable validation stable and rejects semantic source mutation", async () => {
		...
		// The normalized schema is a detached clone, deliberately NOT frozen: the
		// wire-schema post-processor (toolWireSchema) mutates nested nodes in
		// place. Detachment — not immutability — is the invariant under test.
		expect(Object.isFrozen(liveTool?.parameters)).toBe(false);
