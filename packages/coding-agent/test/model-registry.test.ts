...
describe("ModelRegistry", () => {
	...
	describe("baseUrl override (no custom models)", () => {
		...
		test("model authRef keeps exact OAuth credential affinity across retry stages", async () => {
			...
			let rotations = 0;
			authStorage.oauth.accessById = async (provider, credentialId, options) => {
			...
			authStorage.limits.rotate = async () => {
				rotations++;
				return true;
			};
			...
			await expect(resolver({ lastChance: false, error: undefined, signal: undefined })).resolves.toEqual({
				apiKey: "initial-exact-token",
				credentialId: 7,
			});
			await expect(resolver({ lastChance: false, error: new Error("expired"), signal: undefined })).resolves.toEqual(
				{ apiKey: "refreshed-exact-token", credentialId: 7 },
			);
			await expect(
	...
	describe("modelOverrides (per-model customization)", () => {
		...
		test("requires the exact durable OAuth row for pinned model availability", async () => {
			...
			const [pinned, sibling] = authStorage.oauth.accounts(provider);
			...
			expect(await authStorage.credentials.removeById(provider, pinned.credentialId)).toBe(true);
			expect(authStorage.credentials.list(provider).map(row => row.id)).toEqual([sibling.credentialId]);
			expect(
				registry
					.getAvailable()
					.filter(model => model.provider === provider)
