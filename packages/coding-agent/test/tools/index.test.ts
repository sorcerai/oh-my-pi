...
describe("createTools", () => {
	...
	it("gates skill_search on skills settings and read access", async () => {
		const unrestricted = await createTools(
			createTestSession({
				settings: createSettingsWithOverrides({ "skills.enabled": true, "tools.xdev": false }),
			}),
		);
		expect(unrestricted.map(tool => tool.name)).toContain("skill_search");
