...
import * as claudeCodeDiscovery from "@oh-my-pi/pi-catalog/discovery/claude-code";
...
describe("ModelRegistry command-resolved models.yml values", () => {
	...
	beforeEach(async () => {
		...
		// Cache/auth tests must not probe the user's installed Claude CLI.
		spyOn(claudeCodeDiscovery, "fetchClaudeCodeModels").mockResolvedValue([]);
	});
	...
	test.each(["provider:custom-proxy", "oauth-credential:custom-proxy:2"])(
		"401 refreshes command headers without bypassing explicit authRef %s",
		async authRef => {
			const tenantFile = path.join(tempDir, "auth-ref-tenant.txt");
			await Bun.write(tenantFile, "stale-tenant");
			await Bun.write(
				modelsPath,
				JSON.stringify({
					providers: {
						"custom-proxy": {
							baseUrl: "https://custom-proxy.example.com/v1",
							api: "openai-completions",
							apiKey: "wrong-provider-fallback",
							headers: { "x-tenant-token": `!${stdoutFileCommand(tenantFile)}` },
							models: [{ id: "custom-model", name: "Custom Model" }],
						},
					},
			...
			try {
				const seen: Array<{ auth?: string; tenant?: string }> = [];
				const gate = refreshGateFetch(seen);
				...
				const stream = streamSimple(model, context, {
					...
					fetch: async (url, init) => {
						const response = await gate(url, init);
						if (response.status === 401) {
							bearer = "fresh-bearer";
							await Bun.write(tenantFile, "fresh-tenant");
						}
						return response;
					},
					maxTokens: 16,
				...
				for await (const _event of stream) {
					// Drain the real auth-retry stream against the local fetch seam.
				}
