...
//
//  Wire protocol: see README.md (## Protocol).
...
private func handleStreamCancel(_ header: [String: Any]) async {
    let id = (header["id"] as? String) ?? ""
    guard currentSessionId == id else {
        // A late cancel must not tear down a newer active session.
        return
    }
...
/// Bound each raw audio frame to a conservative 4 MiB before allocating or
/// reading it. At Float32 mono 16 kHz this still permits about 65 seconds.
private let maxAudioPayloadBytes = 4 * 1024 * 1024

/// Return a validated raw payload size, or nil for malformed framing.
private func validatedByteCount(_ header: [String: Any], required: Bool) -> Int? {
    guard let value = header["byteCount"] else {
    ...
    guard raw.isFinite,
          ...
          raw.truncatingRemainder(dividingBy: 4) == 0,
          raw <= Double(maxAudioPayloadBytes)
    else {
        return nil
    }
    return Int(raw)
...
private func runWorker() async {
    while true {
        ...
        guard let header = try? JSONSerialization.jsonObject(with: headerData) as? [String: Any] else {
            // Corrupt framing — a later frame boundary is unknowable.
            logErr("stt-nemotron: invalid header JSON; stopping")
            return
        }
        let type = (header["type"] as? String) ?? ""

        // [optional raw payload]
        guard let byteCount = validatedByteCount(
            header,
            required: type == "transcribe" || type == "stream_audio"
        ) else {
            logErr("stt-nemotron: invalid audio byteCount; stopping")
            return
        }
        var payload = Data()
        if byteCount > 0 {
