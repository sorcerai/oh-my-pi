 ...
 * falls back to the default `read`/`grep`/`glob` subset (plus `recall` when the
 * active memory backend provides it); an explicit empty list grants no tools.
 * `instructions` is the advisor's specialization, appended to the shared
 * baseline.
 */
