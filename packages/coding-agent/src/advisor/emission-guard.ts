...
export class AdvisorEmissionHistory {
	...
	add(key: string, rank = 1): void {
		const previous = this.#seen.get(key);
		if (previous !== undefined) {
			if (rank > previous) this.#seen.set(key, rank);
			return;
		}
...
export class AdvisorEmissionGuard {
	/** Shared session history records the highest admitted severity per note. */
	...
	escalatePending(note: string, rank: number): void {
		...
		if (!key) return;
		const seenRank = this.#history.rank(key);
	...
	/** Record the highest admitted rank through the shared bounded history. */
	#recordRank(key: string, rank: number): void {
		this.#history.add(key, rank);
	}
