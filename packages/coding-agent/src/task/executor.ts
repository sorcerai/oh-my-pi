...
// The roster lives in irc/roster so light modules (agent:// peers) avoid importing the executor.
export { collectIrcPeerRoster, type IrcPeerRosterData, type IrcPeerRosterRow } from "../irc/roster";
	...
	const runSubagent = async (): Promise<{
		...
		try {
			...
			const buildSubagentSessionOptions = (
				...
				// Upstream 8cc328a6: prepared extensions pass ungated so restricted and
				// revived children retain parent hooks; sdk.ts binds them with tool
				// admission clamped. The retained parent runtime (above) still wins for
				// unrestricted children via `preloadedExtensions`.
			...
			// device-only write transport for otherwise allowed tools.
			const initialEnabledToolNames = new Set(session.getEnabledToolNames());
			...
			// A startup-injected write is a transport for admitted devices, not a
			// new filesystem grant; keep it available while rejecting hook additions.
			const isParentOwnedTool = (name: string): boolean => !prewalk && name === "todo";
			const isDeniedToolName = (name: string): boolean =>
				isParentOwnedTool(name) ||
				(restrictToolNames &&
					!toolNames?.includes(name) &&
					!(name === "write" && toolNames !== undefined && initialEnabledToolNames.has(name)));
			const subagentToolNames = session.getEnabledToolNames();
			const filteredSubagentTools = subagentToolNames.filter(name => !isDeniedToolName(name));
				...
				extensionRunner.initialize(
					{
						...
						...createTaskExtensionRuntimeActions(session, extensionRunner, isDeniedToolName),
						getActiveTools: () => session.getEnabledToolNames(),
						getAllTools: () => session.getAllToolInfos(),
						setActiveTools: (toolNames: string[]) =>
							session.setActiveToolsByName(toolNames.filter(name => !isDeniedToolName(name))),
