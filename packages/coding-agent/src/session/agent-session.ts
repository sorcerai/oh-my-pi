...
import { selectSkillsForPrompt, type Skill, type SkillWarning } from "../extensibility/skills";
...
export class AgentSession {
	...
	/** Exact prompt-visible skill selection for host-agnostic context accounting. */
	projectPromptSkills(): readonly Skill[] {
		const hasSkillReader = this.getEnabledToolNames().some(name => toolReadsSkillUris(this.getToolByName(name)));
		return selectSkillsForPrompt(this.skills, hasSkillReader, this.skillsSettings);
	}
	...
	async setSkillful(enabled: boolean): Promise<boolean> {
		...
		if (this.agent.state.messages.length === 0) {
			...
			const hasSkillReader = this.getEnabledToolNames().some(name => toolReadsSkillUris(this.getToolByName(name)));
			const renderedSkills = selectSkillsForPrompt(this.skills, hasSkillReader, this.skillsSettings);
	...
	async applyAdvisorConfigs(
		...
		return this.#advisors.applyAdvisorConfigs(advisors, sharedInstructions, sharedMaxNotesPerUpdate, toolPools);
	}
