...
import {
	...
	cfgPrimeBridgeUrl,
} from "./integrations/prime-bridge/settings";
	...
	try {
		...
		const toolSession: ToolSession = {
			...
			externalPeerProvider:
				restrictToolNames || cfgPrimeBridgeEnabled.get(settings) !== true
					? undefined
					: (options.externalPeerProvider ??
						createExternalPeerProvider({
							enabled: true,
							autoStart: cfgPrimeBridgeAutoStart.get(settings),
							url: cfgPrimeBridgeUrl.get(settings),
							tokenPath: cfgPrimeBridgeTokenPath.get(settings),
		...
		const claudeSdkBridge = new ClaudeSdkBridge({
			getApprovalSettings: () => {
				const active = toolContextStore.getContext()?.settings ?? settings;
				return { mode: cfgToolsApprovalMode.get(active), policies: cfgToolsApproval.get(active) };
			},
