...
const ARKTYPE_INTERNAL_KEYS = new Set([
	...
	"traverseAllows",
	"traverseApply",
	// New arktype internals (post-2.x): the compiled IR graph (`ir` is cyclic
	// and carries non-JSON leaves) and runtime helpers are representation, not
	// semantics — fingerprinting them would reject every callable schema.
	"ir",
	"hasSteps",
	"hasDefault",
	"defaultValue",
...
function cloneJsonSchemaAndFreeze<T>(
	...
	for (const key of Reflect.ownKeys(value)) {
		...
		(clone as Record<string, unknown>)[key] = cloneJsonSchemaAndFreeze(descriptor.value, seen, active);
	}
	active.delete(value);
	return clone as T;
}
...
function cloneAndFreeze<T>(value: T, seen = new WeakMap<object, unknown>(), freeze = true): T {
	if (value === null) return value;
	if (typeof value === "function") {
		if (!freeze) return normalizeCallableSchema(value as unknown as CallableArkSchema) as T;
	...
	seen.set(value, clone);
	for (const key of Reflect.ownKeys(value)) {
		// Fresh array clones already carry their own non-configurable `length`; the
		// indexed assignments below resize it. Redefining it explicitly would throw.
		if (Array.isArray(clone) && key === "length") continue;
		const descriptor = Object.getOwnPropertyDescriptor(value, key);
		if (!descriptor || !("value" in descriptor)) continue;
		// The parameters schema must stay writable: the wire-schema post-processor
		// (`toolWireSchema`) mutates nested nodes in place, and a frozen subtree
		// (e.g. arktype's own frozen `toJsonSchema` output) throws on assignment.
		// The clone is still detached from the extension-owned original, and the
		// definition-level freeze below pins the reference itself.
		const childFreeze = freeze && key !== "parameters";
		Object.defineProperty(clone, key, {
			...descriptor,
			value: cloneAndFreeze(descriptor.value, seen, childFreeze),
		});
	}
	return (freeze ? Object.freeze(clone) : clone) as T;
}
...
function callableSchemaSnapshot(
	...
	for (const key of Reflect.ownKeys(source).sort((a, b) => String(a).localeCompare(String(b)))) {
		if (typeof key !== "string") {
			// New arktype instances carry symbol-keyed internals. Their values are
			// cyclic runtime state, not semantics: record presence only, never
			// traverse. Real semantics flow through the string keys below and the
			// normalized JSON Schema.
			ownState.push(`symbol(${key.description ?? ""}):present`);
			continue;
		}
		if (ARKTYPE_INTERNAL_KEYS.has(key) && !Object.hasOwn(ARKTYPE_SEMANTIC_KEYS, key)) continue;
		const descriptor = Object.getOwnPropertyDescriptor(source, key);
		if (!descriptor || !("value" in descriptor)) {
			throw new TypeError(`Callable schema has an accessor property: ${JSON.stringify(key)}`);
		}
...
function isArkTypeCallable(value: unknown): value is CallableArkSchema {
	...
	return (
		...
		(typeof candidate.rootApply === "function" || typeof candidate.apply === "function")
	);
}
