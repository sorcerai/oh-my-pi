...
function parseModelsSnapshot(candidate: string, bytes: Buffer): Record<string, unknown> {
	try {
		const content = bytes.toString("utf8");
		const parsed =
			candidate.endsWith(".json") || candidate.endsWith(".jsonc") ? JSONC.parse(content) : YAML.parse(content);
		const checked = ModelsConfigFile.schema(parsed);
		if (checked instanceof OmpErrors || !isRecord(checked))
			throw new DestinationValidationError("models destination is invalid");
		validateModelsConfig(checked as ModelsConfig);
		return checked;
	} catch (error) {
	...
	for (const candidate of modelCandidatePaths(destination))
		try {
			...
			return {
				...
				value: parseModelsSnapshot(candidate, snapshot.bytes),
			};
		} catch (error) {
...
function privateCredentialFile(stat: Stats, expected?: CredentialIdentity): boolean {
	const expectedIdentity =
		expected !== undefined &&
		stat.dev === expected.dev &&
		stat.ino === expected.ino &&
		stat.mode === expected.mode &&
		(stat.nlink === expected.nlink || stat.nlink === expected.nlink + 1);
	return (
		stat.isFile() &&
		!stat.isSymbolicLink() &&
		(stat.nlink === 1 || expectedIdentity) &&
...
async function credentialFileIdentity(
	dbPath: string,
	expected?: CredentialIdentity,
): Promise<CredentialIdentity | undefined> {
	try {
		const stat = await fs.lstat(dbPath);
		if (!privateCredentialFile(stat, expected)) throw new DestinationValidationError("unsafe credential database");
...
async function credentialStoreIdentity(
	dbPath: string,
	expectedPrimary?: CredentialIdentity,
): Promise<CredentialStoreIdentity> {
	const primary = await credentialFileIdentity(dbPath, expectedPrimary);
...
async function probeCredentialStore(dbPath: string, expectedPrimary?: CredentialIdentity): Promise<CredentialProbe> {
	try {
		...
		const identity = await credentialStoreIdentity(dbPath, expectedPrimary);
...
async function openCredentialReadOnly(dbPath: string, expectedPrimary?: CredentialIdentity): Promise<Database> {
	const identity = await credentialStoreIdentity(dbPath, expectedPrimary);
...
async function credentialProviderDigest(
	dbPath: string,
	provider: string,
	expectedPrimary?: CredentialIdentity,
): Promise<string | undefined> {
	const store = await probeCredentialStore(dbPath, expectedPrimary);
	if (!store.valid || !store.present) return undefined;
	let db: Database | undefined;
	try {
		db = await openCredentialReadOnly(dbPath, expectedPrimary);
		const rows = db
			.query(
...
export async function validatePrimeDestinationRollbackEntry(
	...
	try {
		...
		if (entry.kind === "models") {
			if (entry.nodeType !== "regular-file") return false;
			const canonical = path.resolve(entry.canonicalDestinationRef);
			if (canonical !== path.resolve(destination.modelsPath)) return false;
			const parsed = parseModelItemId(entry.itemId);
			if (!parsed || !(await noSymlinkPath(destination.agentDir, canonical, "file"))) return false;
			const current = await readModels(destination);
			if (path.resolve(current.path) !== canonical || current.sha256 !== entry.currentSha256) return false;
			...
			if (model.id !== undefined && model.id !== parsed.modelId) return false;
			return true;
		}
...
interface CreateOnlyPublication {
	readonly committed: boolean;
	readonly cleanupFailed: boolean;
}
async function publishCreateOnly(stage: string, destination: string): Promise<CreateOnlyPublication> {
	try {
		await fs.link(stage, destination);
	} catch (error) {
		if (isExists(error)) return { committed: false, cleanupFailed: false };
		throw error;
	}
	try {
		...
		try {
			const [staged, published] = await Promise.all([fs.lstat(stage), fs.lstat(destination)]);
			if (
				!staged.isFile() ||
				...
				throw new DestinationValidationError("published credential mismatch");
		} catch (verificationError) {
			if (verificationError instanceof DestinationValidationError) throw verificationError;
			return { committed: true, cleanupFailed: true };
		}
		return { committed: true, cleanupFailed: true };
	}
	try {
	...
	return { committed: true, cleanupFailed: false };
}
	...
	try {
		...
		return await withFileLock(lockPath, async () => {
			...
			if (literal.length) {
				...
				if (!destinationPresent && Object.keys(stagedAfter.companions).length > 0)
				...
				const importedCredentialProviders = batchResult.inserted,
					...
					credentialRollbackEntries: PrimeRollbackManifestEntry[] = [];
				for (const provider of importedCredentialProviders) {
					const authDigest = await credentialProviderDigest(credentialStagePath, provider, stagedAfter.primary);
					if (!authDigest) throw new DestinationValidationError("imported credential missing");
					credentialRollbackEntries.push(
						rollbackEntry(
							`credential:${provider}`,
							"credentials",
				...
				if (!destinationPresent) {
					const publication = await publishCreateOnly(credentialStagePath, plan.destination.agentDbPath);
					if (!publication.committed)
						throw new DestinationValidationError("credential destination appeared during publication");
					committed = true;
					if (publication.cleanupFailed) {
						partialApply = true;
						losses.push(loss("destination-cleanup-failed", "destination", plan.destination.agentDir));
					}
				} else {
					const destinationAfter = await credentialStoreIdentity(
							plan.destination.agentDbPath,
							destinationBefore.primary,
						),
						destinationProbeAfter = await probeCredentialStore(
							plan.destination.agentDbPath,
							destinationBefore.primary,
						);
					...
					if (!sameCredentialStoreIdentity(destinationBefore, destinationAfter))
				...
				if (importedCredentialProviders.length) {
					committed = true;
					rollbackEntries.push(...credentialRollbackEntries);
				}
				credentialDestinationInvalid = false;
			}
