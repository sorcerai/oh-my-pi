...
import { validateProviderMaxInFlightRequests } from "../session/settings";
...
/**
 * One whole-value or per-role create-only settings mutation. `path` is a registered setting id
 * (unknown ids are rejected); whole values are checked like a handle write, and `modelRoles`
 * is only accepted per role.
 */
export type SettingsCreateOnlyMutation =
	| { readonly path: string; readonly role?: undefined; readonly value: unknown }
	| { readonly path: "modelRoles"; readonly role: string; readonly value: string };
...
function hasNonPlainAncestor(obj: RawSettings, segments: readonly string[]): boolean {
	let current: unknown = obj;
	for (let i = 0; i < segments.length - 1; i++) {
...
function canonicalSettingsOsPath(candidate: string): string {
	...
	if (
		...
		return path.resolve("/private", resolved.slice(1));
	}
	return resolved;
}
...
export class Settings {
	...
	async #applyCreateOnly(
		...
		const targets: { identifier: string; segments: readonly string[]; value: unknown; role?: string }[] = [];
		for (const mutation of mutations) {
			const setting = lookupSetting(mutation.path);
			if (!setting) throw new Error(`Unknown setting path: ${mutation.path}`);
			let target: (typeof targets)[number];
			if (mutation.role !== undefined) {
				if (setting !== cfgModelRoles) throw new Error(`Setting ${mutation.path} does not take a role`);
				target = {
					identifier: `modelRoles:${mutation.role}`,
					segments: ["modelRoles", mutation.role],
					value: mutation.value,
					role: mutation.role,
				};
			} else {
				if (setting === cfgModelRoles) throw new Error("Create-only modelRoles mutations must name a role");
				setting.assertWritable(mutation.value);
				target = { identifier: setting.id, segments: setting.segments, value: mutation.value };
			}
			if (identifiers.has(target.identifier)) {
				throw new Error(`Duplicate create-only setting identifier: ${target.identifier}`);
			}
			identifiers.add(target.identifier);
			targets.push(target);
		}

		const previousEffective = this.#snapshot();
		...
		const applyToCurrent = (current: RawSettings): void => {
			// A soft-pinned default is not a configured value: it neither blocks an insert nor survives one.
			const overrides = structuredClone(this.#overrides);
			for (const setting of this.#softPins) deleteByPath(overrides, setting.segments);
			let freshMerged = this.#mergeOwnLayers({ ...this.#ownLayers(), global: current, overrides });

			for (const target of targets) {
				if (
					hasNonPlainAncestor(freshMerged, target.segments) ||
					getByPath(freshMerged, target.segments) !== undefined
				) {
					skipped.push(target.identifier);
					continue;
				}

				setByPath(current, target.segments, target.value);
				applied.push(target.identifier);

				if (target.role !== undefined) {
					freshMerged = this.#deepMerge(freshMerged, { modelRoles: { [target.role]: target.value } });
				} else {
					setByPath(freshMerged, target.segments, target.value);
				}
			}
		...
		const writePathValue = (target: RawSettings, id: string, value: unknown): void => {
		...
		const installAuthoritative = (current: RawSettings): void => {
			...
			for (const modifiedPath of this.#modified) {
				laterPathValues.set(modifiedPath, getByPath(this.#global, modifiedPath.split(".")));
			}
			const laterRoles = [...this.#modifiedGlobalModelRoles];
			const latestRoles = this.#modelRolesFromLayer(this.#global);

			this.#global = current;
			for (const [modifiedPath, value] of laterPathValues) writePathValue(this.#global, modifiedPath, value);
			...
			if (applied.length > 0) this.#persistedMutationGeneration++;
			const layers = this.#ownLayers();
			for (const setting of this.#settlePins(layers)) this.#softPins.delete(setting);
			this.#overrides = layers.overrides;
			this.#rebuildMerged();

			// Listener/effect errors are logged by the registry, never thrown past the commit.
			if (runHooks) this.#fireChangesSince(previousEffective);
		};
		...
		try {
			...
			await this.#withYamlWriteLock(configPath, async writePath => {
				...
				for (const modifiedPath of pendingPaths) {
					writePathValue(current, modifiedPath, getByPath(pendingGlobal, modifiedPath.split(".")));
				}
		...
		try {
			if (limits) {
				...
				try {
					...
					const maxFileBytes = BigInt(limits.maxFileBytes);
					...
					if (BigInt(this.#readTotalBytes) > maxTotalBytes - stat.size)
