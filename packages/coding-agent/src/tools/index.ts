...
import type { ExternalPeerProvider } from "../integrations/prime-bridge";
...
import { cfgSkillsEnabled } from "../extensibility/settings";
...
export interface ToolSession {
	...
	/** Optional provider for Prime peers outside the local AgentRegistry. */
...
export async function resolveBuiltinToolPlan(session: ToolSession, toolNames?: string[]): Promise<BuiltinToolPlan> {
	...
	const isToolAllowed = (name: string) => {
		...
		if (name === "skill_search")
			return (
				cfgSkillsEnabled.get(session.settings) !== false &&
