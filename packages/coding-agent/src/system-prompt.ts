...
export async function buildSystemPrompt(options: BuildSystemPromptOptions = {}): Promise<BuildSystemPromptResult> {
	...
	// Skill selection owns the fork prompt-mode/allowlist filter and hidden-skill drop.
	// The skill:// read gate consults upstream capability metadata (any active tool
	// declaring `readsSkillUris`, including mounted xd:// tools once their metadata
	// is projected) instead of the literal `read` name; the name check only covers
	// the no-metadata fallback.
	const hasSkillReader =
		tools === undefined
			? inventoryToolNames.includes("read")
	...
	const filteredSkills = selectSkillsForPrompt(skills, hasSkillReader, skillsSettings);
