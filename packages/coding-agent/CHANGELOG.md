...
- Local installer smoke checks can select a configured worker with `OMP_LOCAL_SMOKE_AGENT` (default: `flash-worker`).

### Fixed

- Advisors now pause after repeated quota-exhausted recovery attempts instead of retrying indefinitely.
- Restricted subagents and advisors with invalid-only tool lists no longer gain unrequested tools.
...
- Local installer smoke checks recognize verified worker results recovered through `hub jobs` or retried waits after interruptions.
...
- Added task-aware and adversarial advisor roles with isolated per-seat tools and cross-seat duplicate suppression.
