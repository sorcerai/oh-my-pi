...
export interface NonMessageTokenSource {
	...
	/**
	 * Host prompt-selection settings (e.g. skills promptMode/promptSkills).
	 * Identity participates in the memo key so setting swaps re-estimate.
	 */
	readonly skillsSettings?: unknown;
	/**
	 * Skills the host actually renders into the system prompt. Hosts with
	 * prompt-mode selection (allowlists, skill-URI-capable tool gates) project
	 * the discovered set here so accounting stays aligned with the
	 * provider-facing prompt. Absent: the hide-filter mirror below applies.
	 */
	projectPromptSkills?(): readonly ContextSkill[];
}
...
/**
 * Skills actually rendered into the system prompt: hosts with prompt-mode
 * selection project the set via `projectPromptSkills`; the fallback mirrors
 * the base filter in `buildSystemPrompt` (`system-prompt.ts`) — the `read`
 * tool must be present so the model can fetch skill content, and skills with
 * frontmatter `hide: true` (or `disable-model-invocation`, normalized onto
 * `hide`) are excluded. Accounting must count only rendered skills so the
 * Skills category and the System-prompt subtraction stay aligned with the
 * provider-facing prompt.
 */
function defaultRenderedSkills(
...
function renderedSkills(
	...
	return session.projectPromptSkills?.() ?? defaultRenderedSkills(skills, tools);
}
...
interface NonMessageTokenCache {
	...
	skillsSettingsRef: unknown;
	projectPromptSkillsRef: (() => readonly ContextSkill[]) | undefined;
...
function nonMessageTokenCacheEntry(
	...
	const projectPromptSkillsRef = session.projectPromptSkills;
	...
	if (
		...
		entry.projectPromptSkillsRef === projectPromptSkillsRef &&
...
export function computeNonMessageBreakdown(
	...
	const skillsTokens =
		skillful === false
			? 0
			: estimateSkillsTokens(renderedSkills(session, session.skills ?? EMPTY_SKILLS, tools), tokenizer);
