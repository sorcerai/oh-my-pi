...
export interface CoordinationDetails {
	...
	interrupted?: boolean;
	/** Verbatim receipts returned by the Prime bridge for `write agent://prime~<id>`. */
	externalReceipts?: BridgeReceipt[];
	/** Prime bridge message claimed and acknowledged by `wait`. */
...
export const waitToolRenderer = {
	...
	renderResult(
		...
		const external = result.details?.externalWaited;
		if (external && !result.isError) {
			const from = sanitizeText(external.originSessionId).replace(/[\r\n]+/g, " ");
			const body = sanitizeText(external.body);
			return createCachedComponent(
				() => options.expanded,
				(width, expanded) =>
					[
						renderStatusLine(
							{
								...
								title: `Prime ${uiTheme.nav.back} ${replaceTabs(from)}`,
								meta: external.replyTo ? ["reply"] : [],
							},
							uiTheme,
						),
						...bodyLines(body, expanded, uiTheme, { indent: "  " }),
