...
- Bumped the pinned Codex client version to `0.153.0`, which the backend version-gates model availability against; an older pin silently hid newer SKUs, so `openai-codex/gpt-6-astra` is now discovered. Added its compat rule (documented 1.05M window floor, credit-equivalent pricing, free cache writes) and taught multi-account discovery to skip an account whose credential the backend rejected (401/403) instead of vetoing every sibling account's models.
