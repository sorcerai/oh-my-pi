...
async function buildBinary(target: BinaryTarget): Promise<void> {
	...
	if (workerAssetName) {
		// The compiled runtime resolves the worker only beside its executable,
		// so a darwin-arm64 release without this asset ships a broken product.
		// SwiftPM cannot cross-host: refuse to build the target at all rather
		// than silently publish a workerless omp-darwin-arm64.
