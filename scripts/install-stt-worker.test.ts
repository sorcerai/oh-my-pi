...
`,
	...
	// Optional mv stub: can fail a requested final rename and/or the worker
	// backup restore while passing every other operation to the real mv.
	if (options.failRename || options.failWorkerRestore) {
...
${failFinalRename}
${failWorkerRestore}
for real_mv in /bin/mv /usr/bin/mv; do
  [ -x "$real_mv" ] && exec "$real_mv" "$@"
done
