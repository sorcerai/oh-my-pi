...
prompt="Run one nested agent task with the configured agent named $smoke_agent. Give it this instruction: Reply with exactly $marker and no other text, then finish by calling yield with type result and no data, so that reply is the raw result. Do not set an outputSchema on the task. You must call the task tool and wait for its result. If the nested result carries $marker, reply with exactly $marker and no other text. Otherwise, fail without printing the marker."
