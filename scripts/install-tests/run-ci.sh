...
for pkg in utils wire omptype catalog ai mnemopi snapcompact agent tui stats collab-web prime-bridge-protocol prime-bridge; do
   (
...
prime_bridge_protocol_tgz="$(find_tarball "$TARBALL_DIR"/oh-my-pi-prime-bridge-protocol-*.tgz)"
prime_bridge_tgz="$(find_tarball "$TARBALL_DIR"/oh-my-pi-prime-bridge-*.tgz)"
...
(
   ...
   node -e "
		...
		pkg.overrides = {
			...
			'@oh-my-pi/prime-bridge-protocol': '$prime_bridge_protocol_tgz',
			'@oh-my-pi/prime-bridge': '$prime_bridge_tgz'
		};
		require('fs').writeFileSync('package.json', JSON.stringify(pkg, null, 2));
	"

   bun add "$utils_tgz" "$wire_tgz" "$omptype_tgz" "$natives_tgz" "$catalog_tgz" "$ai_tgz" "$mnemopi_tgz" "$snapcompact_tgz" "$agent_tgz" "$tui_tgz" "$stats_tgz" "$coding_agent_tgz" "$collab_web_tgz" "$prime_bridge_protocol_tgz" "$prime_bridge_tgz"
